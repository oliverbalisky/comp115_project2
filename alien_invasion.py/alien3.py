...
class Alien(Sprite):
  ...
  def update(self):
    '''Based on which side of the screen in randomnly chosen, the alien enters the screen and moves diagonally across it '''
    ...
    if self.rect.right < 0 or self.rect.left > self.settings.screen_width or self.rect.bottom < 0 or self.rect.top > self.settings.screen_height:
        # If an alien moves off the screen, remove it.   
        self.kill()
